...
class FeedScreen extends ConsumerWidget {
  ...
  const FeedScreen({super.key, required this.onTapUser, required this.onCreatePost});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.add, size: 16, color: Theme.of(context).brightness == Brightness.dark ? Colors.black : Colors.white),
                const SizedBox(width: 4),
                Text('Post', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: Theme.of(context).brightness == Brightness.dark ? Colors.black : Colors.white)),
              ]),
            ),
          ),
        ],
      ...
      body: state.loading
          ? ListView.builder(
              itemCount: 4,
              itemBuilder: (ctx, i) => const SkeletonCard().animate(delay: Duration(milliseconds: i * 80)).fadeIn(),
            )
